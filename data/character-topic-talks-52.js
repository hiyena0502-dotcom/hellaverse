...
(()=>{
  ...
  const TOPIC_ECHO=[
    ...
    t=>"‘"+t+"’이라는 표현 자체가 이미 여러 상황을 한데 묶고 있어.",
    t=>"내가 이 화제를 다르게 보는 이유도 결국 ‘"+t+"’의 조건이 매번 달라서야.",
    ...
    t=>"이 화제에서 제일 위험한 건 ‘"+t+"’을 너무 쉽게 일반화하는 거야.",
    t=>"결국 ‘"+t+"’도 상황이 바뀌면 같은 답을 유지하기 어렵지.",
    t=>"‘"+t+"’을 말할 때는 누가 손해를 떠안는지도 같이 봐야 해.",
    t=>"나는 ‘"+t+"’을 듣자마자 규칙보다 실제 사람부터 떠올리게 돼.",
    t=>"‘"+t+"’은 말로 정리한 원칙과 실제 행동이 가장 자주 어긋나는 주제 중 하나야."
  ];
  const PLAYER_ECHO=[
    (c,t)=>"특히 "+c.name+"처럼 이 문제와 직접 맞닿은 입장이라면 더 다르게 보일 것 같아요.",
    (c,t)=>"당신이 방금 보여준 반응 때문에 ‘"+t+"’을 그냥 일반론으로 묻고 싶진 않네요.",
    (c,t)=>"지금 당신이 있는 위치를 생각하면 ‘"+t+"’은 남의 얘기처럼 들리진 않겠죠.",
    (c,t)=>"평소 당신이 중요하게 보는 기준이 ‘"+t+"’에서도 그대로 적용되는지 궁금해요.",
    (c,t)=>"이 화제는 당신 성격상 남들이랑 같은 답이 나오진 않을 것 같아요.",
    (c,t)=>"‘"+t+"’을 당신 방식대로 풀면 어떤 부분부터 달라지는지 듣고 싶어요.",
    (c,t)=>"방금까지 한 얘기를 보면 ‘"+t+"’에서도 우선순위가 꽤 분명할 것 같네요.",
    (c,t)=>"당신은 ‘"+t+"’을 말할 때 감정보다 먼저 보는 게 따로 있을 것 같아요.",
    (c,t)=>"같은 질문을 다른 사람에게 했을 때와는 전혀 다른 답이 나올 것 같아서 물어요.",
    (c,t)=>"‘"+t+"’ 같은 문제에서 당신이 절대 양보하지 않는 선이 뭔지 궁금해졌어요.",
    (c,t)=>"당신 경험을 빼고 ‘"+t+"’을 말하긴 어려울 것 같네요.",
    (c,t)=>"이건 다른 사람 의견 말고 정말 당신 쪽 판단을 듣고 싶어요.",
    (c,t)=>"‘"+t+"’이라는 주제가 당신한테는 어디서 개인적인 문제가 되는지도 궁금해요."
  ];
  ...
  const make=(c,t,n)=>{
    ...
    const echo=TOPIC_ECHO[(hash(id+"echo")+n)%TOPIC_ECHO.length](t.title);
