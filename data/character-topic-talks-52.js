  ...
  const topicTalkPhrase=(title,r="casual")=>{
    ...
    if(unchanged||lacksSentence){
      const fallback=[
        ...
        [/천국|천사|엑소시스트|숙청/,"천국 쪽에서 이런 일 생기면 반응 꽤 크겠지?"],
        [/영혼|계약|오버로드/,"계약이나 권력 얘기는 한번 꼬이면 꽤 복잡해지지?"],
        [/방송|뉴스|미디어|라디오|텔레비전|복스|벨벳/,"방송 쪽은 이런 거 하나로도 말 엄청 나오겠지?"],
        [/I\.M\.P|의뢰|임무|회사|직원/,"일하다 이런 일 생기면 꽤 골치 아프겠지?"],
        [/Goetia|귀족|가문|왕족/,"귀족 쪽은 사소한 일도 금방 커지잖아?"],
        [/연애|사랑|관계|결혼|전 애인/,"관계 얘기는 밖에서 보는 것보다 훨씬 복잡하지?"],
        [/가족|부모|자녀|아버지|형제/,"가족 얘기는 사소한 것도 쉽게 커지지?"],
        [/인간|Sinner|Hellborn|Ring/,"사는 곳이나 출신이 다르면 당연한 것도 꽤 달라지지?"],
        [/파티|술|게임|휴가|놀이|노래/,"이런 건 그냥 가볍게 즐기면 되는 거 아니야?"],
        [/법|규칙|권력|통치|서열/,"이런 건 결국 누가 결정하느냐가 문제겠지?"],
        [/무기|싸움|죽|폭력|협박|현상수배/,"이건 실제로 터지면 누가 먼저 움직일까?"],
        [/음식|요리|조식|메뉴/,"먹는 얘기는 결국 직접 해봐야 알지 않을까?"],
        [/친구|비밀|신뢰|믿/,"사람 사이 일은 말보다 반응이 더 정확하지?"]
      ];
      const hit=fallback.find(([re])=>re.test(raw));
      if(hit)s=hit[1];
      else{
        const core=raw
          .replace(/에 대한 .*$/,"")
          ...
          .replace(/는 .*$/,"")
          .trim();
        const short=(core&&core.length<=16?core:raw.split(/\s+/).slice(0,2).join(" "))
          .replace(/(을|를|은|는|이|가)$/,"");
        s=(short||"이런 얘기")+" 쪽은 막상 닥치면 생각이 좀 달라지지?";
      }
    }
