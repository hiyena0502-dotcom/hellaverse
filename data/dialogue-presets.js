...
(()=>{
  ...
  window.HV_APPLY_DIALOGUE_PRESETS=(source,helpers={})=>{
    ...
    if(current<13&&pruneRetiredSoloTalk(source))changed=true;
    if(current<13&&syncTunedStoryPacks(source))changed=true;
    if(current<13&&repairLegacyCharacterRefs(source))changed=true;
    if(current<13){
