...
function renderConditions(o,kind,attrs=""){
  ...
  '<div class="condition-grid"><span>정확 호감도 구간</span><input '+attrs+' type="number" min="0" max="100" placeholder="최소" data-affcond-kind="'+kind+'" data-affcond-field="minValue" value="'+(ac.minValue??"")+'"><input '+attrs+' type="number" min="0" max="100" placeholder="최대" data-affcond-kind="'+kind+'" data-affcond-field="maxValue" value="'+(ac.maxValue??"")+'"><span></span></div>'+
        ...
        '<div class="ask-row interaction-editor-row editor-single-detail" data-ask-id="'+esc(a.id)+'">'+
          ...
          '<div class="full-row interaction-response-editor">'+
            ...
            '<div class="interaction-effect-grid">'+
              ...
              '<label class="field"><span>재질문 시 호감도 변화</span><input type="number" min="-100" max="100" data-ask-bind="repeatAffectionDelta" value="'+(a.repeatAffectionDelta||0)+'"></label>'+
            ...
            '</div><label class="checkline"><input type="checkbox" data-ask-bind="applyAskDeltaOnce" '+(a.applyAskDeltaOnce?"checked":"")+'> 최초 질문 변화값은 한 번만 적용 (재질문 변화는 매번 적용)</label>'+interactionFlowEditor(a.entries,"ask",a.id)+
          '</div>'+
        '</div>'
...
function renderGachaEditor(){
  ...
  '<div class="gacha-profile-editor-grid">'+profiles.map(({ch,profile,count})=>'<article class="gacha-profile-editor-card" data-gacha-profile-character="'+esc(ch.id)+'"><span class="gacha-profile-icon-preview">'+esc(profile?.icon||"🎴")+'</span><div class="gacha-profile-fields"><div class="gacha-profile-card-head"><strong>'+esc(ch.name)+'</strong><small>'+count+' ITEMS</small></div><label class="field"><span>아이콘</span><input data-gacha-profile-field="icon" data-gacha-profile-character="'+esc(ch.id)+'" value="'+esc(profile?.icon||"🎴")+'" placeholder="예: 🎴"></label><label class="field"><span>GACHA 제목</span><input data-gacha-profile-field="title" data-gacha-profile-character="'+esc(ch.id)+'" value="'+esc(profile?.title||ch.name+" GACHA")+'" placeholder="'+esc(ch.name)+' GACHA"></label><label class="field full"><span>설명</span><textarea data-gacha-profile-field="description" data-gacha-profile-character="'+esc(ch.id)+'" placeholder="이 캐릭터의 컬렉션/가챠 설명">'+esc(profile?.description||"")+'</textarea></label></div></article>').join("")+'</div></div>'+
...
function renderThoughtEditor(){
  ...
  editorBody.innerHTML=editorHead("THOUGHT","Thought 설정","목록에서 한 문장만 선택해 편집합니다.",'<button class="small-button" data-action="new-thought">+ Thought</button>')+
    ...
    '<div class="manager-layout editor-select-layout"><aside class="manager-list"><div class="manager-list-items">'+
      (visible.length?visible.map(x=>'<button class="manager-item '+(x.id===selectedThoughtId?"active":"")+'" data-action="select-thought" data-id="'+esc(x.id)+'"><strong>'+esc((x.text||"빈 문장").slice(0,52))+'</strong><small>'+esc(getCharacterDraft(x.characterId)?.name||"캐릭터 미지정")+' · '+esc(x.category)+' · 호감도 '+Number(x.minAffection||0)+'~'+Number(x.maxAffection??100)+'</small></button>').join(""):'<div class="editor-note">검색 결과가 없습니다.</div>')+
